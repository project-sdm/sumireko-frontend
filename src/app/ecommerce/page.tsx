...
export default function Ecommerce() {
  ...
  async function handleSearch() {
    ...
    setError(null);
    try {
      const data = await searchSimilar(file, k);
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-16 flex flex-col gap-10">
      <header className="text-center">
        <h1 className="text-4xl font-semibold tracking-tight">Sumireko</h1>
      ...
      {preview && (
        <div className="flex items-center justify-center gap-4">
          <div className="flex items-center gap-2 rounded-lg bg-surface px-3 py-2">
            <label className="text-sm text-foreground/60">Results</label>
            <button
          ...
          <button
            ...
            disabled={loading}
            className="rounded-lg bg-accent px-5 py-2 text-sm font-medium text-accent-foreground transition-colors hover:opacity-90 disabled:opacity-50 cursor-pointer"
          >
